    ...
    <DashboardLayout showNav={false}>
      <div className="flex flex-col h-[calc(100vh-48px)]">
        <div className="flex items-center gap-3 pb-4">
          ...
          <h1 className="text-[22px] font-bold" style={{ color: 'var(--text-primary)' }}>Breathing</h1>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          ...
          <div className="relative w-72 h-72 flex items-center justify-center mb-10">
            ...
            <motion.div
              ...
              transition={{ duration: phase === 'hold' ? 7 : phase === 'inhale' ? 4 : 8, ease: 'easeInOut' }}
              className="w-40 h-40 rounded-full dash-card flex items-center justify-center"
            >
              <Wind size={40} className="text-[var(--accent-teal)]" />
            </motion.div>
          </div>
          ...
          >
            ...
            {isActive && (
              <p className="text-[40px] font-bold text-[var(--accent-teal)]">{count}</p>
            )}
          </motion.div>
          ...
          <div className="flex items-center gap-6">
            <button
              ...
              className="w-20 h-20 rounded-full dash-card flex items-center justify-center dash-interactive"
            >
              {isActive ? <Pause size={28} style={{ color: 'var(--text-primary)' }} /> : <Play size={28} style={{ color: 'var(--text-primary)' }} className="ml-1" />}
            </button>
          </div>
